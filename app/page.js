...
export default async function Home() {
  ...
  if (!profile) return <div className="p-10 text-primary text-center">Data kosong. Jalankan 'node prisma/seed.js' dulu.</div>

  return (
    <main className="min-h-screen bg-base text-secondary font-sans selection:bg-accent selection:text-primary pb-20">
            ...
            <div key={edu.id} className="bg-elevated p-6 md:p-8 rounded-2xl border border-border hover:bg-accent-soft transition-colors duration-300 flex flex-col md:flex-row gap-6">
              ...
              <div className="md:w-3/5 flex flex-col">
                <div className="flex justify-between items-start mb-3">
                    ...
                    {/* Periode di Mobile */}
                    <span className="md:hidden text-xs font-mono text-muted bg-elevated px-3 py-1 rounded-full border border-border whitespace-nowrap">
                        {edu.period}
                    </span>
                </div>
                ...
                {gpaFullText && (
                  <div className="p-4 bg-accent-soft border border-accent/20 rounded-xl w-fit">
                    <p className="text-sm text-accent uppercase tracking-wider font-bold mb-1">
                      {gpaFullText.includes('GPA') ? 'Cumulative GPA' : 'Final Score'}
                    </p>
                    <p className="text-3xl md:text-4xl font-extrabold text-accent tracking-tight leading-none">
                      {gpaFullText.split(': ')[1]}
                    </p>
                  </div>
                )}
              ...
              <div className="md:w-2/5 flex flex-col items-end justify-between border-t md:border-t-0 md:border-l border-border pt-4 md:pt-0 md:pl-6">
                 <span className="hidden md:inline-block text-sm font-mono text-muted bg-elevated px-3 py-1 rounded-full border border-border whitespace-nowrap w-fit mb-4">
                    {edu.period}
                 </span>
                 ...
                 {edu.imageUrl && (
                   ...
                   <div className="relative h-40 w-full rounded-xl overflow-hidden shadow-md border border-border bg-white p-3 mt-auto">
                     <img
                       src={edu.imageUrl}
                       alt={edu.school}
          ...
          {Object.values(
            ...
            <div key={index} className="flex flex-col md:flex-row gap-8 bg-elevated p-6 rounded-2xl border border-border hover:bg-accent-soft transition-colors duration-300">
              ...
              <div className="w-full md:w-1/3 shrink-0">
                <div className="relative h-48 w-full rounded-xl overflow-hidden shadow-lg border border-border bg-white p-2"> 
                  <img 
                    src={group.imageUrl} 
                    alt={group.company}
                    className="w-full h-full object-contain" 
              ...
              <div className="w-full md:w-2/3 flex flex-col justify-center">
                <h3 className="text-accent font-bold text-xl mb-6">{group.company}</h3>
                
                <div className="space-y-8 relative before:absolute before:inset-y-0 before:left-1 before:w-0.5 before:bg-border">
                    ...
                    <div key={role.id} className="relative pl-8">
                      ...
                      <div className="flex flex-col mb-2">
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-sm mt-1">
                          ...
                          <span className="px-2 py-0.5 bg-accent-soft text-accent text-xs rounded border border-accent/40">
                            {role.type}
                          </span>
                        </div>
                      </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {certifications.map((cert) => (
              <div key={cert.id} className="bg-elevated p-6 rounded-xl border border-border hover:border-accent transition-colors duration-300 flex flex-col">
                ...
                <p className="text-muted text-sm mt-auto">{cert.issuer}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <footer className="py-8 text-center border-t border-border mt-20">
        <p className="text-muted text-sm mb-2">
          Designed & Built by <span className="text-accent font-bold">David Christian Nathaniel</span>
        </p>
        ...
        <div className="flex justify-center gap-6 mt-4 opacity-50 hover:opacity-100 transition-colors duration-300">
          ...
          {profile.github && (
             <a href={`https://${profile.github}`} target="_blank" className="text-muted hover:text-primary text-sm">GitHub</a>
           )}

           <a href={`mailto:${profile.email}`} className="text-muted hover:text-accent-hover text-sm">Email</a>
        </div>
      </footer>
